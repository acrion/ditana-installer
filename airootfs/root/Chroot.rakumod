...
sub generate-root-script() is export {
    my $s = Settings.instance;
    my $script-path = '/mnt/usr/lib/ditana/initialize-system-as-root';
...
sub get-session-setup-script-path returns Str {
    Settings.instance.get("real-install")
        ?? "/mnt/usr/lib/ditana/session-setup"
        !! "/tmp/ditana-session-setup";
}

sub generate-session-setup() is export {
    my $s = Settings.instance;
    my @once-scripts = $s.get-first-login-scripts();
    my @always-scripts = $s.get-login-scripts();

    # Generate /usr/lib/ditana/session-setup which sources all registered scripts
    my $setup-script-path = get-session-setup-script-path();
    my $setup-content = "#!/bin/bash\n";
    $setup-content ~= "# Generated by Ditana installer — sources login scripts before the DE starts.\n\n";
    ...
    Logging.log("Generated login scripts wrapper with {@once-scripts.elems + @always-scripts.elems} external script references");
}
...
sub generate-session-setup-script() is export {
    ...
    unless $setup-script-path.IO.e {
        die "login scripts wrapper does not exist. "
    ...
    Logging.log("Updated login scripts wrapper with {@once-lines.elems + @always-lines.elems} inline command line(s)");
}
...
sub patch-lightdm-conf() is export {
    ...
    if $conf.IO.e {
        ...
        # Set session-wrapper
        if $content !~~ /'session-wrapper=/usr/lib/ditana/session-wrapper'/ {
            if $content ~~ /^^ '#'? \s* 'session-wrapper=' \N* $$/ {
                $content = $content.subst(
                    /^^ '#'? \s* 'session-wrapper=' \N* $$/,
                    'session-wrapper=/usr/lib/ditana/session-wrapper'
                );
            } else {
