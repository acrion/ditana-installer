...
sub create-kernel-configuration() is export {
    ...
    for $s.get-sysctl-values -> $entry {
        $conf-content ~= "$entry[0]=$entry[1] # $entry[2]\n";
    }
    ...
    # The list of executables that may create an unprivileged user namespace.
    # It is written only when something asked for one: with no file,
    # ditana-userns-guard.service does not start, and the machine keeps
    # kernel.unprivileged_userns_clone at the value written above.
    my @userns-allow = $s.get-userns-allow-for-enabled-settings;
    if @userns-allow {
        mkdir("/mnt/etc/ditana");
        "/mnt/etc/ditana/userns-allow.conf".IO.spurt(
            "# Generated by the Ditana installer. One executable per line.\n"
            ~ "# Each entry was declared by a setting that needs a user namespace.\n"
            ~ @userns-allow.join("\n") ~ "\n");
        Logging.log("userns allowlist: {@userns-allow.join(', ')}");
    }
