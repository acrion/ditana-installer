...
=begin tomty
%(
  tag => ["mkinitcpio", "busybox", "without-zfs", "with-encrypt-root-partition","without-nvidia-but-no-nouveau"]
);
=end tomty
...
task-run "tasks/run-ansible", %(
  ...
  vars => "path=/tmp/mkinitcpio.conf zfs_filesystem=n encrypt_root_partition=y use_init_systemd=n nvidia_but_no_nouveau=n",
);
